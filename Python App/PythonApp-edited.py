...
def setup_file_for_reference(current_latest):
    import numpy as np
    
    df = pd.read_csv('/homes/sl222/Documents/individual_proj/IEEEEuroSP23/attack_pattern/enterprise-techniques-with-procedureV2-3v11.csv')
    ...
    df['Name_embedding'] = df['Name'].apply(get_embedding_processed)
    df['Description_embedding'] = df['Description'].apply(get_embedding_processed)
    df['Procedure_embedding'] = df['Procedure'].apply(lambda x: get_embedding_processed(x) if pd.notnull(x) else np.nan)
    # df.to_pickle("enterprise-techniques-with-procedureV4-includeembedding.pkl")
    # df.to_pickle("enterprise-techniques-with-procedureV2-includeembeddingfinal.pkl")
    # df.to_csv("enterprise-techniques-with-procedureV2-includeembedding.csv")
    # df.to_pickle("enterprise-techniques-with-procedureV3-includeembeddingfinal.pkl")
    # df.to_pickle("enterprise-techniques-with-procedureV2-3-includeembeddingfinal.pkl")
    df.to_pickle("v" + current_latest + ".pkl")
