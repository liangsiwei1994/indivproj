...
df = pd.read_csv('/homes/sl222/Documents/individual_proj/IEEEEuroSP23/attack_pattern/enterprise-techniqueslatest24August2023.csv')
...
def get_mitre_id(text):
    ...
    for k, tech_list in attack_pattern_dict.items():
        for v in tech_list:
            ...
            d = (0.4*get_embedding_distance(text, v[0]) + 0.6*get_embedding_distance(text, v[1]))
            if d < min_dist:
                min_dist = d
                ret = k
    return ret, min_dist
