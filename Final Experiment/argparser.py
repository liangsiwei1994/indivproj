...
def parse_bert_sentence_classification_arguments():
    ...
    parser.add_argument('--dataset', default='/homes/sl222/Documents/individual_proj/IEEEEuroSP23/attack_pattern/data/sentence_classification', type=str,
                        help='data directory containing train/dev/test files')
    parser.add_argument('--cuda', default=True, type=lambda x: (str(x).lower() == 'true'), help='use cuda if available')
    parser.add_argument('--lr', default=1e-5, type=float, help='learning rate')
...
def parse_inference_arguments():
    ...
    parser.add_argument('--input-doc', default="samples/", type=str, help='input document path')
    parser.add_argument('--save-path', default='samples/', type=str, help='path for output document')
    args = parser.parse_args()
    return args
